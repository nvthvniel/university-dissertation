...
class service_helper:
    ...
    def extract_payload(self, user_prompt, arguments):
        ...
        for argument in arguments:
            ...
            try:
                index = user_prompt.index(argument_name)

                indexes[argument_name] = {"type": argument_type, "start": index, "end": index+len(argument_name)}
            
            except ValueError:
                indexes[argument_name] = {"type": argument_type, "start": None, "end": None}
        ...
        for a in range (0, len(indexes)):
            ...
            try:
                start = indexes[ arguments[a].split("=")[0] ]["end"]
                end = indexes[ arguments[a+1].split("=")[0] ]["start"]

                if ( (start != None) and (end != None) ):
                    value = user_prompt[start:end].strip(" ")
            
            except IndexError:
                start = indexes[ arguments[a].split("=")[0] ]["end"]

                if (start != None):
                    value = user_prompt[start:].strip(" ")
            

            argument_type = indexes[ arguments[a].split("=")[0] ]["type"]
            ...
            payload[ arguments[a].split("=")[0] ] = value
            logging.info(f" | {arguments[a].split("=")[0]}: {value}")
