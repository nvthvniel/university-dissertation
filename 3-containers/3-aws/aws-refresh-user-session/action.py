...
class action:
        ...
        for target in self.payload["--targets"]:
            ...
            try:
                ...
                if error.response['Error']['Code'] == 'NoSuchEntity':
                    logging.info(f" | Error: Couldn't find users matching specified Names: {self.payload['--targets']} ...")

                    self.service_helper.send_error_message("Couldn't find all users matching specified names")


                    response_code.append(500)
    ...
    def action(self, target):
        ...
        revoke_time = now.strftime("%Y-%m-%dT%H:%M:%S.000Z")

        policy_name = f"jarvis-refresh-user-session-{target}-{timestamp}"
        ...
        if (response_code != 200):
            ...
            self.service_helper.send_error_message(f"Couldn't revoke sessions")

            response_code = 500
            response_data = {"error": "couldn't revoke sessions"}
        
        else:
            logging.info(f" | Applied deny all in-line policy")

            attachments_text = f"*Sessions revoked for \"{target}\" with \"{policy_name}\" in-line policy*"
        
            attachments = [{"text": attachments_text, "color": "#4DAB9A"}]
            self.service_helper.send_response(attachments)

            response_code = 200
            response_data = {"success": "revoked sessions"}
